...
namespace CGAlgorithms.Algorithms.ConvexHull
{
    public class DivideAndConquer : Algorithm
    {
        ...
        public List<Point> merge(List<Point> aa, List<Point> bb)
        {
            ...
            {
                found = true;
                while (HelperMethods.CheckTurn(new Line(b[upperb].X,
                ...
                if (found == true &&
                    (HelperMethods.CheckTurn(new Line(b[upperb].X, b[upperb].Y, a[uppera].X, a[uppera].Y),
                         a[(uppera + 1) % n1]) == Enums.TurnType.Colinear))
                    uppera = (uppera + 1) % n1;

                while (HelperMethods.CheckTurn(new Line(a[uppera].X, a[uppera].Y, b[upperb].X, b[upperb].Y),
                           b[(n2 + upperb - 1) % n2]) == Enums.TurnType.Left)
                {
                ...
                if (found == true &&
                    (HelperMethods.CheckTurn(new Line(a[uppera].X, a[uppera].Y, b[upperb].X, b[upperb].Y),
                         b[(upperb + n2 - 1) % n2]) == Enums.TurnType.Colinear))
            ...
            {
                found = true;
                while (HelperMethods.CheckTurn(new Line(b[lowerb].X, b[lowerb].Y, a[lowera].X, a[lowera].Y),
                           a[(lowera + n1 - 1) % n1]) == Enums.TurnType.Left)
                {
                ...
                if (found == true &&
                    (HelperMethods.CheckTurn(new Line(b[lowerb].X, b[lowerb].Y, a[lowera].X, a[lowera].Y),
                         a[(lowera + n1 - 1) % n1]) == Enums.TurnType.Colinear))
                    lowera = (lowera + n1 - 1) % n1;

                while (HelperMethods.CheckTurn(new Line(a[lowera].X, a[lowera].Y, b[lowerb].X, b[lowerb].Y),
                           b[(lowerb + 1) % n2]) == Enums.TurnType.Right)
                {
                    lowerb = (lowerb + 1) % n2;
                    found = false;
                ...
                if (found == true &&
                    (HelperMethods.CheckTurn(new Line(a[lowera].X, a[lowera].Y, b[lowerb].X, b[lowerb].Y),
                         b[(lowerb + 1) % n2]) == Enums.TurnType.Colinear))
                    lowerb = (lowerb + 1) % n2;
